...
LFS=${QTGATE_ROOT:-$B/rootfs-lfs-parity}   # x86: 패리티 rootfs (변환기가 이 형태를 못 잡아 1차 사슬이 ARM64 경로를 봤음)
...
else [[ "$LFS" == *"x86-parity"* ]] || { echo "🚨 ABORT: $LFS"; exit 1; }; fi
...
if false; then   # x86 네이티브: binfmt 불필요
