...
if false; then   # x86: glibc SVE(ARM) 헤더 패치 해당 없음
  cp -n "$MVH" "$MVH.orig"
...
if [ -n "$MISS" ]; then
  ...
  for pc in $MISS; do
    ...
    env PKG_CONFIG_SYSROOT_DIR="$LFS" PKG_CONFIG_LIBDIR="$LFS/usr/lib/pkgconfig:$LFS/usr/lib64/pkgconfig:$LFS/usr/share/pkgconfig" CC="x86_64-linux-gnu-gcc --sysroot=$LFS" CXX="x86_64-linux-gnu-g++ --sysroot=$LFS" ./configure --host=x86_64-linux-gnu --prefix=/usr --disable-static >> "$LOG" 2>&1 && make -j"$JOBS" >> "$LOG" 2>&1 && make install DESTDIR="$LFS" >> "$LOG" 2>&1 || { echo "🚨 ABORT: $KPKG 빌드 실패"; tail -20 "$LOG"; exit 1; }
...
XKBFIX
  ...
  export PKG_CONFIG_LIBDIR="$LFS/usr/lib/pkgconfig:$LFS/usr/lib64/pkgconfig:$LFS/usr/share/pkgconfig"
  ...
  # (sed 한 줄에 한 행 — \n 이스케이프 함정 #32 회피)
  MKS="mkspecs/linux-g++-64/qmake.conf"
  ...
  # x86 전용: linux-g++-64 mkspec의 X11R6 경로(QMAKE_INCDIR_X11/QMAKE_LIBDIR_X11 = /usr/X11R6/*)가 sysroot 밖이라
  # configure의 xlib(makeSpec) 검사가 "Library path /usr/X11R6/lib64 is invalid"로 실패 → 두 줄 제거(기본 경로 = sysroot /usr)
  sed -i '/X11R6/d' "$MKS"   # INCDIR/LIBDIR_X11 + LIBDIR_OPENGL 전부 /usr/X11R6/*
  grep -q 'X11R6' "$MKS" && { echo "🚨 ABORT: mkspec X11R6 잔재"; exit 1; }
  echo "  ✓ mkspec X11R6 경로 제거 (x86)" | tee -a "$LOG"
  echo "  ✓ mkspec FORTIFY=2 주입" | tee -a "$LOG"

  if [ -f Makefile ] && [ -f .qmake.cache ]; then
    echo "  (재사용) 기존 configure 결과 — make만 재개" | tee -a "$LOG"
  else
  rm -f config.cache   # x86: 이전 실패(xlib) 검사 결과가 캐시돼 재검사를 막는다
...
[ -x "$LFS/usr/bin/qterminal" ] && { echo "  ✅ qterminal"; readelf -h "$LFS/usr/bin/qterminal" | grep -q 'X86-64' && echo "  ✅ qterminal AArch64" || { echo "  🚨 qterminal 아키텍처 오류"; ok=0; }; } || echo "  ⏳ qterminal 미빌드"
# 아키텍처 확인 — 크로스가 아니라 x86을 만들었으면 즉시 발각
if [ -e "$LFS/usr/lib/libQt5Core.so" ]; then
  readelf -h "$LFS/usr/lib/libQt5Core.so" | grep -q 'X86-64' && echo "  ✅ x86-64 ELF" || { echo "  🚨 libQt5Core가 AArch64가 아님"; ok=0; }
fi
