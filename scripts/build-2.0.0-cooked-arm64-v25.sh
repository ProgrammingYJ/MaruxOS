#!/bin/bash
###############################################################################
# MaruxOS 2.0.0 "Cooked" — ARM64 이미지 빌드 v25 (독 유령창 근절 + 경고창 가시화)
#
# v24 대비 변경 (2026-08-25 실기기 피드백 — 호버 ✅ / 목록 아이콘 ✅ / 나머지 2건 교정):
#   ① **독 유령창 근절: 창을 override-redirect로** — DOCK(v22)·POPUP_MENU(v23) 모두
#      "창 타입 힌트"일 뿐이라 WM이 관리하는 창이고, bamf가 인식해 plank에 항목이 남았다.
#      `Gtk.WindowType.POPUP`(override-redirect)은 **WM이 아예 관여하지 않아** bamf가
#      존재를 모른다. 바·드롭다운 모두 전환(키 입력이 필요한 비밀번호 창만 일반 창 유지).
#   ② **경고창 가시화** — 부모 없는 모달이 화면 뒤/밖에 뜨던 문제:
#      CENTER_ALWAYS + keep_above + stick + show_all/present. 비밀번호 창도 동일 처리
#      (부모를 override-redirect 패널로 두면 배치가 깨지므로 독립 최상위 창으로).
#   (v24 유지: cairo 호버·목록 아이콘·통합 상태 바·한영 엔진 패치·커널 FWSUP·WiFi 스택)
#   🐛 조사 중: plank 우클릭 "닫기" 무동작(event_time 가설) / 한영 표시(ibus 엔진은 입력
#      포커스 시에만 기동 — xterm에 커서를 두고 타이핑하는 상태에서 Shift+Space 검증 필요).
#
...
trap cleanup EXIT
echo "===== MaruxOS 2.0.0 ARM64 v25 (유령창 근절 + 경고창) 빌드 $(date) ====="
