...
build_autotools(){   # $1=이름 $2=버전 $3=URL $4...=configure 옵션
  ...
  env PKG_CONFIG_SYSROOT_DIR="$LFS" PKG_CONFIG_LIBDIR="$LFS/usr/lib/pkgconfig:$LFS/usr/share/pkgconfig" \
      CC="aarch64-linux-gnu-gcc --sysroot=$LFS" CXX="aarch64-linux-gnu-g++ --sysroot=$LFS" CFLAGS="-O2 -fcommon -D_FORTIFY_SOURCE=2" CXXFLAGS="-O2 -fcommon -D_FORTIFY_SOURCE=2" \
...
if [ -x "$LFS/usr/bin/pcmanfm-qt" ]; then
  readelf -h "$LFS/usr/bin/pcmanfm-qt" | grep -q AArch64 && echo "  ✅ AArch64 ELF" || { echo "  🚨 아키텍처 오류"; ok=0; }
fi
