#!/bin/bash
# MaruxOS 2.0.0 "Cooked" - cooked-v7 Build Script
# =================================================
# v6 부팅 결과: Plank schema 해결 (libplank-common) → 떴음! ✓ 단 빈 dock (launcher 0개) + 부유 위치.
# v7 = Windows 11 스타일 dock 정공 완성:
#   - **Plank dockitem 3개 영구 박음** (xterm/mc/firefox) — config/plank/dock1/launchers/
#   - Plank 설정 (Position=3 bottom, Alignment=0 center, IconSize=48, ZoomEnabled=true)
#   - /etc/skel/.config/plank/ + /root/.config/plank/ 둘 다 배포 (Live ISO + 설치 모두 커버)
#   - tint2 → systray-only 모드 복귀 (v3 분기 부활). plank가 launcher+taskbar 역할.
#
# v6 변경사항 유지: install-plank.sh의 libplank-common 추가.
...
# ============================================================================
# [7.5] Plank dock (2.0.0 cooked-v3 신규 — UI 핵심, 실패 시 strict abort)
# ============================================================================
echo "[7.5] Installing Plank dock..."
if [ -f "$SQUASHFS_ROOT/usr/bin/plank" ]; then
    echo "  ✓ plank already installed"
elif [ -f "$SCRIPT_DIR/install-plank.sh" ]; then
    if ! bash "$SCRIPT_DIR/install-plank.sh" "$SQUASHFS_ROOT"; then
        echo ""
        echo "ERROR: install-plank.sh failed."
        echo "  UI lock 작업 단계에선 Plank가 critical → strict abort."
        echo "  단독 디버그: sudo bash $SCRIPT_DIR/install-plank.sh"
        echo "  silent fail 회피용으로 plank 없이 빌드하고 싶으면 PLANK_OPTIONAL=1 으로 ${PLANK_OPTIONAL:-strict} 모드 우회 (현재 strict)."
        if [ "${PLANK_OPTIONAL:-0}" != "1" ]; then
            exit 1
        fi
        echo "  ⚠ PLANK_OPTIONAL=1, 무시하고 진행 (tint2 default mode)"
    fi
else
    echo "ERROR: install-plank.sh not found at $SCRIPT_DIR"
    exit 1
fi
...
# v7: Plank dock 정상 작동 검증됨 → tint2 systray-only 모드 복귀 (v3 분기 부활)
if [ -f "$SQUASHFS_ROOT/usr/bin/plank" ] && [ -f "$CONFIG_DIR/tint2/tint2rc-systray" ]; then
    cp "$CONFIG_DIR/tint2/tint2rc-systray" "$SQUASHFS_ROOT/etc/xdg/tint2/tint2rc"
    echo "  ✓ tint2 → systray-only mode (Plank dock 활성, Windows 11 스타일)"
else
    cp "$CONFIG_DIR/tint2/tint2rc" "$SQUASHFS_ROOT/etc/xdg/tint2/tint2rc"
    echo "  ✓ tint2 → full panel mode (Plank 미설치 시 폴백)"
fi
chmod 644 "$SQUASHFS_ROOT/etc/xdg/tint2/tint2rc"

# Plank dock 설정 배포 — Live ISO root + 신규 사용자 (skel) 둘 다 커버
if [ -d "$CONFIG_DIR/plank" ]; then
    # /etc/skel/.config/plank/ (신규 사용자)
    mkdir -p "$SQUASHFS_ROOT/etc/skel/.config/plank"
    cp -a "$CONFIG_DIR/plank/." "$SQUASHFS_ROOT/etc/skel/.config/plank/"
    # /root/.config/plank/ (Live ISO autologin root)
    mkdir -p "$SQUASHFS_ROOT/root/.config/plank"
    cp -a "$CONFIG_DIR/plank/." "$SQUASHFS_ROOT/root/.config/plank/"
    echo "  ✓ Plank dock config (xterm/mc/firefox) deployed to /etc/skel + /root"
fi
