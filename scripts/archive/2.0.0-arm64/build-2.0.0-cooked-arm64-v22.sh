#!/bin/bash
###############################################################################
# MaruxOS 2.0.0 "Cooked" — ARM64 이미지 빌드 v22 (WiFi 정복 박제 + 우상단 통합 상태 바)
#
# v21 대비 변경 (2026-08-23 실기기 WiFi 성공 + UI 통합):
#   ① **커널 FWSUP 패치 박제** — brcmfmac이 펌웨어 supplicant(sup_wpa)를 감지·활성화하면
#      EAPOL이 host로 오지 않아 4-way가 영원히 끝나지 않는다(함정 #31). feature.c에서
#      FWSUP 감지를 제거해 host supplicant 강제. **실기기 검증 완료**:
#      wpa_state=COMPLETED / ip 192.168.45.179 / 유선 없이 google.com 42ms.
#      커널 SHA 13bd3415… (rebuild-kernel-wifi-arm64.sh [2.5]가 멱등 적용)
#   ② **wpa offload capability 차단** — driver_nl80211_capa.c에서 4WAY_HANDSHAKE_PSK/8021X
#      수용부 무력화(마커 .w-markers/wpa-offload-patched).
#   ③ **우상단 통합 상태 바** (사용자 확정 설계, Windows 트레이 레퍼런스):
#      `한/A · WiFi · 볼륨 │ 오후 2:57 / 2026-08-23`를 **하나의 유리 바**에. 시계를
#      marux-quicksettings로 흡수 → **tint2 은퇴**(config v10, 게이트 6종 정리).
#   ④ **GUI IP 표시 버그 수정** — HashTable 파싱에서 ip_address가 유실되던 문제(사용자 신고)
#      → 직접 파싱 + `ip addr` 폴백 + 3초×3회 재확인("IP 받는 중…" 표시). 스캔 파서도 동일
#      패턴이라 배열 기반으로 선제 교체.
#   (v21 유지: root=PARTUUID + fstab LABEL + RESET_GPIO + 펌웨어 임베드 + UI v2 + 볼륨 부트스트랩)
#   🐛 이월: 독 인디케이터 점 위치/크기(plank 소스), 한영 표시 정확도(엔진명 기준 MVP).
#
...
trap cleanup EXIT
echo "===== MaruxOS 2.0.0 ARM64 v22 (WiFi 정복 + 통합 상태 바) 빌드 $(date) ====="
