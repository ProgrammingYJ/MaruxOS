#!/bin/bash
###############################################################################
# MaruxOS 2.0.0 "Cooked" — ARM64 이미지 빌드 v21 (WiFi 4-way 정공: wpa BRCM 빌드 + UI 개편 + 볼륨 부트스트랩)
#
# v20 대비 변경 (2026-08-23 v19/v20 실기기 피드백 — WiFi 연결 실패 근원 + UI):
#   ① **wpa_supplicant `CONFIG_DRIVER_NL80211_BRCM=y` 재빌드** — v20 실기기 실측:
#      연결(assoc)·암호협상(WPA2-PSK/CCMP)까지 정상인데 `ASSOC INFO: wait for driver
#      port authorized indication` 후 10초 타임아웃 → CONN_FAILED → SSID-TEMP-DISABLED.
#      EAPOL 프레임 **0건**(grep -c eapol = 0) = wpa가 4-way를 펌웨어 offload로 위임했으나
#      brcmfmac 펌웨어가 port-authorized를 끝내주지 않음. Raspberry Pi OS 공식 빌드와
#      동일하게 BRCM 확장을 켜서 해소. 바이너리에 BRCM vendor 문자열 50건 실측.
#      🆕 함정 #30: full-mac 칩은 "비번 틀림"이 WRONG_KEY가 아니라 CONN_FAILED로 위장된다.
#      (부수 발견: 평문 passphrase일 때 nl80211 CONNECT에 PSK가 실리지 않음 — hex PSK로는 전달됨)
#   ② **퀵설정 UI v2** — 사용자 피드백("구석에 딱 붙음/버튼 안 둥금/볼륨 무반응"):
#      가장자리 여백 8→16 + 패널 내부 margin 16, WiFi·볼륨 **카드 계층**(반투명 레이어),
#      **CSS 우선순위 PRIORITY_USER**(Adwaita가 border-image/box-shadow로 라운드를 덮던 문제),
#      목록 행 라운드+호버, 슬라이더 커스텀(얇은 트랙+원형 핸들), 볼륨 % 표시.
#   ③ **볼륨 부트스트랩** — softvol "Master"는 첫 재생 때 생성되므로 xinitrc에서 무음 1초
#      재생(aplay /dev/zero)해 컨트롤을 미리 만든다. (v19: amixer scontrols 빈 출력 실측)
#   (v20 내용 유지: root=PARTUUID + fstab LABEL + RESET_GPIO 커널 + 펌웨어 임베드)
#   🐛 이월: 독 인디케이터 점 위치/크기(plank 소스 패치), 5GHz는 규제 적용 후 재검증.
#
...
trap cleanup EXIT
echo "===== MaruxOS 2.0.0 ARM64 v21 (wpa BRCM + UI v2) 빌드 $(date) ====="
