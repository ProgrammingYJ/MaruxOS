#!/bin/bash
# =============================================================================
# MaruxOS 2.0.0 ARM64 — 데스크톱 config populate v14 (FeatherPad 텍스트 편집기 + MIME 기본앱)
# -----------------------------------------------------------------------------
...
for d in xterm mc firefox qterminal pcmanfm-qt featherpad; do
  cp -f "$CFG/applications/$d.desktop" "$LFS/usr/share/applications/$d.desktop"
done
echo "  ✓ xterm/mc/firefox/qterminal/pcmanfm-qt/featherpad.desktop"
...
table Icon
  Caption: Text Editor
  Command: featherpad
  Icon: /usr/share/pixmaps/maruxos/file-text.png
  X: 80
  Y: 380
  Width: 48
  Height: 48
...
echo "[7b] MIME 기본앱 (text/* → featherpad)"
...
echo "  .desktop    : $(ls "$LFS/usr/share/applications/"{xterm,mc,firefox,qterminal,pcmanfm-qt,featherpad}.desktop 2>/dev/null | wc -l)/6"
