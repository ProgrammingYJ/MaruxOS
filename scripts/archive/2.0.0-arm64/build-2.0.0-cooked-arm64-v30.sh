#!/bin/bash
###############################################################################
# MaruxOS 2.0.0 "Cooked" — ARM64 이미지 빌드 v30 (자동 로그인 + X 자동 기동 — 출품용 "부팅하면 바로 화면")
#
# v29 대비 변경 (2026-08-26, 8/27 출품 직전 — 사용자: "ㄹㅇ 운영체제처럼 부팅하면 바로 화면까지"):
#   ① **config v13**: inittab tty1 `agetty --autologin root` + /root/.bash_profile(tty1이면 startx).
#      시리얼 ttyS0 getty는 유지(디버깅 통로). X 종료/실패 시 tty1 셸로 복귀(루프 없음), 로그아웃 시 재자동로그인.
#   ② 게이트: inittab tty1 autologin 줄 정확 일치 + ttyS0 줄 존재 + .bash_profile(root/skel) startx 가드 + 문법.
#   (v29 유지: Qt FORTIFY=2 + 기동 게이트, .desktop 내용 게이트, PCManFM-Qt, QTerminal, WiFi, 상태 바, 한영)
#   🐛 이월: plank 우클릭 "닫기", 독 인디케이터 점 위치.
#
# 사용법: 전원 인가 → (로그인 프롬프트 없이) 데스크톱 → 독 3종 → QTerminal/PCManFM-Qt → 한글 → 회귀.
###############################################################################
set -euo pipefail
BUILD_TARGET="arm64"
...
trap cleanup EXIT
echo "===== MaruxOS 2.0.0 ARM64 v30 (자동 로그인 + X 자동 기동) 빌드 $(date) ====="
