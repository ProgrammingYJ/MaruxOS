#!/bin/bash
###############################################################################
# MaruxOS 2.0.0 "Cooked" — ARM64 이미지 빌드 v32 (편집기 아이콘 + 배치 T: LXImage-Qt·SpeedCrunch·LXQt Archiver·qps)
#
# v31 대비 변경 (2026-08-27 출품 당일):
#   ① 편집기 아이콘 marux-editor.png (config v15).
#   ② **배치 T** (install-lxtools-arm64.sh): 이미지 뷰어+스크린샷 / 계산기 / 압축 관리자(+json-glib, unzip/zip 비치명) /
#      GUI 작업관리자. .desktop 4종 + 메뉴 + MIME(image/* → lximage-qt, 압축 → lxqt-archiver). 기동 게이트 4종 추가.
#   게이트: 4종 AArch64 + .desktop **10종** config 바이트 일치 + MIME + .t-COMPLETE + 아이콘 5종 실존.
#
# v31 대비 변경 (2026-08-27): config v15 — FeatherPad 아이콘 file-text.png(플레이스홀더) → marux-editor.png
#   ("줄 있는 종이 + 접힌 모서리", 사용자 선택, PIL 생성). featherpad.desktop·idesk editor.lnk 동시 교체.
#   게이트: 기존 .desktop 6종 바이트 일치 + Icon 파일 실존이 그대로 잡는다 + marux-editor.png 실존 명시.
#   (v31 유지: FeatherPad+MIME, 자동 로그인, Qt FORTIFY=2, 독 아이콘, PCManFM-Qt, QTerminal, WiFi, 상태 바, 한영)
...
trap cleanup EXIT
echo "===== MaruxOS 2.0.0 ARM64 v32 (편집기 아이콘 + 배치 T 툴 4종) 빌드 $(date) ====="
