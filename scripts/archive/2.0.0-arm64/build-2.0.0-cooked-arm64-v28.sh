#!/bin/bash
###############################################################################
# MaruxOS 2.0.0 "Cooked" — ARM64 이미지 빌드 v28 (Qt 즉사 픽스: FORTIFY 오탐 — 함정 #35)
#
# v27 대비 변경 (2026-08-26 — v27 실기기 검증에서 발각):
#   ① **qterminal 기동 즉시 SIGABRT / pcmanfm-qt 종료 시 SIGABRT** 픽스.
#      증상: `*** buffer overflow detected ***`. 백트레이스(chroot gdb, 실기기 동일):
#        __readlink_chk ← qt_readlink ← QLockFile ← QSettings ← Properties::migrate_settings
#      원인: Ubuntu 크로스 gcc의 **암묵 -D_FORTIFY_SOURCE=3** + Qt 5.15.2 QByteArray 인라인
#      데이터 포인터 → 동적 크기 추정 오판(실제 오버플로 없음). --version은 QSettings 전에
#      끝나서 v26/v27 게이트(존재·아키텍처)가 못 잡았다.
#      픽스: Qt 스택 전체(qtbase·x11extras·qtermwidget·qterminal·libfm-qt·pcmanfm-qt·libexif·
#      libfm-extra·menu-cache)를 **-D_FORTIFY_SOURCE=2**로 재빌드 (rebuild-qt-fortify-arm64.sh).
#   ② **게이트 승격: 존재 검사 → 기동 검사.** gate-qt-launch-arm64.sh가 rootfs를 qemu chroot로
#      실제 실행(qterminal offscreen 12초 생존 + pcmanfm-qt 세션버스 기동·SIGTERM 종료 경로).
#      + 재빌드가 기록한 통과본 SHA($B/.q-FORTIFY2-OK)와 rootfs 바이너리 대조.
#   (v27 유지: PCManFM-Qt, QTerminal, WiFi 스택, 통합 상태 바, 한영 표시, PARTUUID/LABEL)
#   🐛 이월: plank 우클릭 "닫기", 독 인디케이터 점 위치.
#
# 사용법: 부팅 → startx → 독 1번(QTerminal)·2번(PCManFM-Qt) 기동 → Qt 앱 한글 입력 → 회귀.
###############################################################################
set -euo pipefail
BUILD_TARGET="arm64"
...
trap cleanup EXIT
echo "===== MaruxOS 2.0.0 ARM64 v28 (Qt FORTIFY 픽스 + 기동 게이트) 빌드 $(date) ====="
