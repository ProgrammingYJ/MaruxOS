#!/bin/bash
# =============================================================================
# MaruxOS 2.0.0 ARM64 — 데스크톱 config populate v9 (플로팅 시계 박제)
# -----------------------------------------------------------------------------
# v8 대비 델타 (2026-08-14 실기기 라이브 튜닝 확정 — 사용자 스케치 구현):
#   ① tint2 — clock-only 밀착 바(sed 조합) → **플로팅 시계 위젯**으로 교체.
#      확정 자산 config/tint2/tint2rc-floating 을 그대로 복사 (sed 없음 — 자산이 최종값).
#      = 우하단 공중 라운드 회색 박스(#c8c8c8 90, rounded 12, margin 10),
#        2줄 HH : MM / mm-dd(요일) (NanumGothic Bold 15/10), shrink+패딩30 중앙정렬.
#      v17 실기기에서 라이브 튜닝으로 확정한 값 — Pi 라이브 상태와 1:1 일치 (MD5 6714794428fdff28f419c5cd3309aa84).
#   ② xinitrc — **marux-quicksettings 기동** (배치 W: 우상단 인디케이터 바 + 퀵설정
#      드롭다운. WiFi(wpa_cli)+볼륨(amixer). 바이너리 없거나 wlan0 없어도 무해 가드).
#   나머지는 v8 동일 (bamfdaemon 명시기동, dmesg -n 1, picom, keyfile, HW커서 등).
...
[ -f "$CFG/tint2/tint2rc-floating" ] || { echo "🚨 ABORT: tint2rc-floating 자산 없음"; exit 1; }
...
# ---------- 6. tint2 — v9: 플로팅 시계 위젯 (확정 자산 그대로, sed 없음) ----------
echo "[6] tint2rc-floating (우하단 공중 라운드 회색 박스 시계)"
mkdir -p "$LFS/etc/xdg/tint2"
cp -f "$CFG/tint2/tint2rc-floating" "$LFS/etc/xdg/tint2/tint2rc"
echo "  ✓ tint2rc = floating clock ($(grep '^background_color' "$LFS/etc/xdg/tint2/tint2rc"))"
...
# ---------- 7. full xinitrc — v8 동일 (keyfile + picom + bamf + plank + tint2) ----------
...
# 플로팅 시계 tint2 (우하단 공중 라운드 박스 — strut 없음, 항상 위)
...
echo "  tint2 플로팅: $(grep '^panel_shrink' "$LFS/etc/xdg/tint2/tint2rc") / $(grep '^background_color' "$LFS/etc/xdg/tint2/tint2rc")"
