#!/bin/bash
###############################################################################
# MaruxOS 2.0.0 "Cooked" — ARM64 이미지 빌드 v27 (🏆 로드맵 4/4 완주: PCManFM-Qt)
#
# v26 대비 변경 (2026-08-25 — 2.0.0 로드맵 마지막 항목 달성):
#   ① **PCManFM-Qt 0.17.0 탑재** (+libfm-qt 0.17.0, menu-cache 1.1.0, libfm-extra 1.3.2,
#      libexif 0.6.24) — 파일관리자를 mc에서 교체. 독·데스크톱 아이콘·openbox 메뉴 전환.
#      **mc는 잔류**(터미널 파일관리 + 폴백). 1.x의 PCManFM(GTK) 사고 — GLib 2.68 요구로
#      glibc를 덮어써 시스템을 망가뜨린 그 사건 — 을 **Qt 경로로 정공 해소한 것**이 의미.
#   ② config v12: .desktop 5종, 독 = qterminal·pcmanfm-qt·firefox.
#   (v26 유지: Qt5+QTerminal, WiFi 스택, 통합 상태 바, 한영 표시, PARTUUID/LABEL 부팅)
#   🐛 이월: plank 우클릭 "닫기", 독 인디케이터 점 위치.
#
#   ※ 2.0.0 로드맵: ①ARM64 데스크톱+한글 ②WiFi+퀵설정 ③QTerminal ④PCManFM-Qt — **4/4 완료**
#
...
trap cleanup EXIT
echo "===== MaruxOS 2.0.0 ARM64 v27 (PCManFM-Qt — 로드맵 4/4) 빌드 $(date) ====="
