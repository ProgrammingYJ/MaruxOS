#!/bin/bash
###############################################################################
# MaruxOS 2.0.0 "Cooked" — ARM64 이미지 빌드 v19 (v18 실기기 픽스 3종: RESET_GPIO+alsa-utils+GUI 배경)
#
# v18 대비 변경 (2026-08-14 v18 실기기 검증 피드백 3종 근원 픽스):
#   ① **커널 CONFIG_RESET_GPIO=y** — v18 실기기 실측: "wifi-pwrseq deferred: reset control
#      not ready" → mmc1(WiFi SDIO 호스트) 미기동 → wlan0 부재. 6.18 pwrseq_simple은
#      reset 컨트롤러 프레임워크 우선인데 DT reset-gpios→reset 브리지(RESET_GPIO)가 =m
#      (no-modules 트랩). 새 Image SHA 10ff2fe4… (52,947,456 B — 크기 동일, 패딩 흡수).
#   ② **alsa-utils 1.2.10** — amixer 부재로 퀵설정 볼륨 백엔드 죽어있었음(% 고정 버그).
#      + /etc/asound.conf softvol("Master" 컨트롤 생성 — vc4-hdmi는 HW 볼륨 無).
#   ③ **퀵설정 GUI 재작업** — 투명 패널 버그: CSS 배경 미발현 → **cairo 직접 페인트**
#      (라운드 유리, 플로팅 시계와 동일 #c8c8c8 톤 + 다크 텍스트) + 믹서 컨트롤 동적
#      감지(Master 하드코딩 제거, 컨트롤 부재 시 슬라이더 비활성+"♪ --").
#   (v18 내용물 전부 유지: WiFi 커널 임베드 + wpa 2.11 + 퀵설정 + 플로팅 시계 config v9)
#   🐛 미탑재(알려진 버그, 다음 패치 이월): 독 인디케이터 점 위치/크기 — plank 소스 패치.
#
# 사용법: 부팅 → startx → 우상단 바 클릭 → WiFi 스캔/연결 + 볼륨 + 플로팅 시계 + 회귀.
###############################################################################
set -euo pipefail
BUILD_TARGET="arm64"
...
IMGSIZE="27G"
KIMG_SHA_EXPECTED="10ff2fe4d3a5eca3d5a3eede271d1ef91423d579671c9ee121871fafb874fc14"
MNT=""; LOOP=""
cleanup(){ set +e; [ -n "$MNT" ] && mountpoint -q "$MNT" && umount "$MNT"; [ -n "$LOOP" ] && losetup -d "$LOOP" 2>/dev/null; [ -n "$MNT" ] && [ -d "$MNT" ] && rmdir "$MNT" 2>/dev/null; }
trap cleanup EXIT
echo "===== MaruxOS 2.0.0 ARM64 v19 (배치 W 픽스 3종) 빌드 $(date) ====="
