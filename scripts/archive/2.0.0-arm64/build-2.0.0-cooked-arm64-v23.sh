#!/bin/bash
###############################################################################
# MaruxOS 2.0.0 "Cooked" — ARM64 이미지 빌드 v23 (한영 정확표시 + WiFi 아이콘 + UX 픽스)
#
# v22 대비 변경 (2026-08-23 실기기 피드백 4건):
#   ① **한/영 정확 표시** — ibus-hangul은 Shift+Space로 *엔진 내부 모드*만 바꿔 `ibus engine`
#      으로는 구분 불가(v22까지 "한" 고정). ibus 패널 D-Bus 구현 대신 **엔진 소스를 패치**해
#      모든 전환이 지나는 단일 지점(set_input_mode)에서 `/tmp/marux-ime-mode`에 han/eng 기록
#      → GUI는 파일만 읽는다. (patch-ibus-hangul.sh, 엔진 재빌드 169,864 B)
#   ② **WiFi 신호를 아이콘으로** — 폰트에 WiFi 글리프가 없어(이모지 금지 함정) **cairo 직접
#      드로잉**: 호 3개+중심점, RSSI(signal_poll) 기반 0~3단계, 끊기면 전체 흐림.
#   ③ **독에 뜨던 유령 창 제거** — 드롭다운 패널이 UTILITY 타입이라 bamf가 창으로 잡아
#      plank에 "marux-quicksettings" 항목이 생겼다 → **POPUP_MENU**로 변경.
#   ④ **바 호버 하이라이트**(마우스 올린 영역만 밝게) + **연결 실패 알림**(그동안 "연결 중…"
#      에서 멈춰 원인을 알 수 없었음) + **TEMP-DISABLED 자동 해제**(한 번 실패하면 재시도가
#      막히던 문제) + **부팅 시 규제 도메인 KR 자동 적용**(5GHz 채널 개방 시도).
#   (v22 유지: 커널 FWSUP 패치 + wpa offload 차단 + 통합 상태 바 + tint2 은퇴 + PARTUUID/LABEL)
#   🐛 이월: 독 인디케이터 점 위치/크기(plank 소스 패치).
#
...
trap cleanup EXIT
echo "===== MaruxOS 2.0.0 ARM64 v23 (한영 정확표시 + WiFi 아이콘) 빌드 $(date) ====="
