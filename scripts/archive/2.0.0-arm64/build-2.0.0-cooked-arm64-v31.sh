#!/bin/bash
###############################################################################
# MaruxOS 2.0.0 "Cooked" — ARM64 이미지 빌드 v31 (FeatherPad 텍스트 편집기 + MIME 기본앱)
#
# v30 대비 변경 (2026-08-27 출품 당일 — 사용자: "txt 여는 notepad 같은 거 포함"):
#   ① **배치 E: FeatherPad 0.17.1** (Qt5, install-featherpad-arm64.sh) + **qtsvg** 모듈(FeatherPad 필수).
#      hunspell 없음 → 맞춤법 OFF. FORTIFY=2 체계 그대로(함정 #35), 기동 게이트에 featherpad 추가.
#   ② **config v14**: featherpad.desktop + openbox 메뉴 "Text Editor" + 바탕화면 아이콘 + **MIME 기본앱**
#      (mimeapps.list ×2 + 직접 생성한 mimeinfo.cache) → PCManFM-Qt에서 .txt 더블클릭 = FeatherPad.
#   ③ 게이트: featherpad AArch64 + libQt5Svg + .desktop 6종 config 바이트 일치 + mimeapps/mimeinfo + .e-COMPLETE.
#   (v30 유지: 자동 로그인+X 자동 기동, Qt FORTIFY=2, 독 아이콘, PCManFM-Qt, QTerminal, WiFi, 상태 바, 한영)
#   🐛 이월: plank 우클릭 "닫기", 독 인디케이터 점 위치.
#
# 사용법: 전원 인가 → 데스크톱 → 메뉴/바탕화면 "Text Editor" → PCManFM-Qt에서 .txt 더블클릭 → 한글 입력.
###############################################################################
set -euo pipefail
BUILD_TARGET="arm64"
...
trap cleanup EXIT
echo "===== MaruxOS 2.0.0 ARM64 v31 (FeatherPad + MIME) 빌드 $(date) ====="
...
echo "  ✓ .desktop 6종 = config 원본 (바이트 일치) + Icon 파일 실존 + idesk 아이콘 실존"
