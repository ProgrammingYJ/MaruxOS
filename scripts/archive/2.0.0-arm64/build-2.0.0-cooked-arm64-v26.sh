#!/bin/bash
###############################################################################
# MaruxOS 2.0.0 "Cooked" — ARM64 이미지 빌드 v26 (🏆 배치 Q: Qt5 + QTerminal 탑재)
#
# v25 대비 변경 (2026-08-25 — 2.0.0 로드맵 3번 달성):
#   ① **Qt5 5.15.2 크로스 빌드 탑재** — MaruxOS 최초의 호스트 크로스 컴파일(qemu 네이티브는
#      Qt 규모상 불가). Core/Gui/Widgets/DBus/Network/Sql/Xml/Concurrent/X11Extras/XcbQpa +
#      **xcb 플랫폼 플러그인**(GUI 표시의 전제). 10겹의 벽 상세 = 함정 #33/#34.
#   ② **QTerminal 0.17.0** (+qtermwidget 0.17.0, lxqt-build-tools 0.13.0) — 기본 터미널을
#      xterm에서 교체. 독·데스크톱 아이콘·openbox 메뉴 전부 qterminal로 통일(xterm은
#      **폴백으로 잔류** — Qt 스택 이상 시 대피로).
#   ③ **config v11**: `QT_QPA_PLATFORM_PLUGIN_PATH=/usr/plugins/platforms` + `QT_QPA_PLATFORM=xcb`
#      명시(플러그인 탐색 실패 시 Qt 앱이 기동조차 못 하므로 예방).
#   (v25 유지: WiFi 스택·통합 상태 바·한영 표시·override-redirect 창·PARTUUID/LABEL 부팅)
#   🐛 이월: plank 우클릭 "닫기"(event_time 가설), 독 인디케이터 점 위치.
#
...
trap cleanup EXIT
echo "===== MaruxOS 2.0.0 ARM64 v26 (Qt5 + QTerminal) 빌드 $(date) ====="
