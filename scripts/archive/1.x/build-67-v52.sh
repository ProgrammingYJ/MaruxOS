#!/bin/bash
# MaruxOS 67-v52 Build Script (ibus-daemon foreground execution for debugging)
...
echo "ibus-daemon foreground execution"
echo "=========================================="
...
# Apply v52 changes (Remove --daemonize for debugging)
echo "[4/11] Copying config files..."

# Copy xinitrc (with foreground ibus-daemon execution)
echo "  - Copying xinitrc with foreground ibus-daemon..."
...
echo ""
echo "Changes in v52:"
echo "  🔧 --daemonize 제거하고 포그라운드(&) 실행"
echo "  🔧 실제 에러 메시지가 /tmp/ibus-daemon.log에 기록됨"
echo "  🔧 PID 추적 추가 (DAEMON_PID)"
echo ""
echo "Debugging improvements:"
echo "  - ibus-daemon을 백그라운드(&)로 실행"
echo "  - --daemonize 옵션 제거로 실제 에러 확인 가능"
echo "  - 프로세스 PID 로그 기록"
echo "  - /tmp/ibus-daemon.log에 상세한 에러 메시지 출력 예상"
echo ""
echo "Next steps after boot:"
echo "  1. cat /tmp/ibus-debug.log    # 메인 디버그 로그"
echo "  2. cat /tmp/ibus-daemon.log   # ibus-daemon 실제 에러 메시지"
echo "  3. ps aux | grep ibus         # 프로세스 확인"
echo ""
echo "Note: This build should reveal the actual crash reason"
echo "      Check /tmp/ibus-daemon.log for real error messages"
echo "=========================================="
