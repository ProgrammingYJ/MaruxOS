#!/bin/bash
# MaruxOS 67-v51 Build Script (ibus-daemon debugging enhanced)
...
echo "ibus-daemon debugging enhanced"
echo "=========================================="
...
# Apply v51 changes (Enhanced ibus-daemon debugging)
echo "[4/11] Copying config files..."

# Copy xinitrc (with enhanced ibus-daemon debugging)
echo "  - Copying xinitrc with enhanced ibus debugging..."
...
echo ""
echo "Changes in v51:"
echo "  🔧 ibus-daemon 디버깅 강화"
echo "  🔧 ldd로 의존성 확인 추가"
echo "  🔧 --verbose 모드로 상세 로그 수집"
echo "  🔧 별도 로그 파일 (/tmp/ibus-daemon.log) 생성"
echo "  🔧 exit code 확인 추가"
echo "  🔧 프로세스 체크 로직 개선"
echo "  🔧 sleep 5초로 증가"
echo ""
echo "Debugging features:"
echo "  - /tmp/ibus-debug.log: 메인 디버그 로그"
echo "  - /tmp/ibus-daemon.log: ibus-daemon 상세 로그"
echo "  - ldd 의존성 체크"
echo "  - ibus-daemon exit code 확인"
echo "  - 모든 ibus 프로세스 확인"
echo ""
echo "Note: Built on top of v50 (Ctrl+Y for Korean input switching)"
echo "      Enhanced debugging to identify why ibus-daemon crashes"
echo "      Check /tmp/ibus-daemon.log for detailed error messages"
echo "=========================================="
