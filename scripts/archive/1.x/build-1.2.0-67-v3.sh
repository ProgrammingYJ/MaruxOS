#!/bin/bash
# MaruxOS 1.2.0-67-v3 Build Script
# Bugfix: New File/Folder/Script 또는 Refresh Desktop 후 모든 바탕화면 아이콘 사라지는 버그 수정
#   - 원인: ( idesk & ) 서브셸 패턴이 세션 분리를 못해서 xterm 종료 시 SIGHUP에 idesk가 같이 죽음
#   - 수정: setsid로 새 세션/프로세스 그룹 생성 → SIGHUP 차단
#   - 위치: config/scripts/marux-new-desktop-item, config/openbox/menu.xml (Refresh Desktop)
