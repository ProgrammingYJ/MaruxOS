#!/bin/bash
# MaruxOS 2.0.0 "Cooked" - cooked-v3 Build Script
# =================================================
# v2 부팅 검증 통과 (메타데이터 깔끔, 한글입력 OK) 후,
# UI를 Windows 11 스타일 dock으로 개편.
#
# v3 변경사항 (vs cooked-v2):
#   - **Plank dock 도입** — Debian Bookworm prebuilt .deb 추출 (1.x Firefox 패턴)
#     의존성: libplank, libgee-0.8, libwnck-3, libgnome-menu-3 추가
#   - tint2 역할 축소: launcher+taskbar 영역 제거, systray + clock만 (slim 모드)
#     → tint2rc-systray로 교체. 하단 우측에 collapsed 패널.
#   - xinitrc에 plank 자동 시작 추가 (conditional: /usr/bin/plank 있을 때만)
#   - .desktop 파일들 StartupWMClass 정정 (xterm/firefox — plank 없이 tint2 모드에서도 효과 있음)
#
