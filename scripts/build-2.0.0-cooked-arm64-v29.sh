#!/bin/bash
###############################################################################
# MaruxOS 2.0.0 "Cooked" — ARM64 이미지 빌드 v29 (독 아이콘 투명 픽스: 패키지 재설치가 config를 덮어씀)
#
# v28 대비 변경 (2026-08-26 — v28 실기기 검증에서 발각):
#   ✅ v28 성과 확인: QTerminal·PCManFM-Qt **둘 다 기동됨**(독 실행 점 표시) — 함정 #35 픽스 실증.
#   ① **독/데스크톱 아이콘 투명** 픽스. 원인: Qt FORTIFY 재빌드의 `make install`이 config v12가
#      배포한 `/usr/share/applications/{qterminal,pcmanfm-qt}.desktop`을 **업스트림 원본으로 덮어씀**
#      (Icon=utilities-terminal / system-file-manager — 테마 이름인데 hicolor뿐이라 빈 아이콘).
#      "config → rootfs" 흐름의 역방향 오염: **패키지 재설치 후엔 config 재적용이 필수**.
#      픽스 = config v12 재적용(파일 내용 원복). 스크립트 변경 없음.
#   ② **게이트 승격 "존재 ≠ 내용"**: 독 5종 `.desktop`이 `config/applications/` 원본과 **바이트 일치**해야
#      하고, 절대경로 Icon은 파일이 실존해야 한다. idesk `.lnk`의 Icon 파일도 실존 검사.
#      (v28 게이트는 .desktop *존재*만 봤다 — 함정 #35의 "존재≠기동"과 같은 계열)
#   (v28 유지: Qt FORTIFY=2 스택 + 기동 게이트, PCManFM-Qt, QTerminal, WiFi, 통합 상태 바, 한영, PARTUUID)
#   🐛 이월: plank 우클릭 "닫기", 독 인디케이터 점 위치.
#
# 사용법: 부팅 → startx → 독 아이콘 3종 표시 → QTerminal/PCManFM-Qt 기동·닫기 → Qt 한글 → 회귀.
###############################################################################
set -euo pipefail
BUILD_TARGET="arm64"
...
trap cleanup EXIT
echo "===== MaruxOS 2.0.0 ARM64 v29 (독 아이콘 픽스 + .desktop 내용 게이트) 빌드 $(date) ====="
...
echo "  ✓ .desktop 5종 = config 원본 (바이트 일치) + Icon 파일 실존 + idesk 아이콘 실존"
