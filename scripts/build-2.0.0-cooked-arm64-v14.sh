#!/bin/bash
###############################################################################
# MaruxOS 2.0.0 "Cooked" — ARM64 이미지 빌드 v14 (배치 P: Plank dock)
#
# v13 대비 변경:
#   ① **Plank 0.11.89 소스 빌드 스택** — vala 0.56.17 부트스트랩(자체 valac!) +
#      g-i/libgee/libXres/libwnck/libgtop/gnome-menus/xcb-util/startup-notification/bamf.
#      x86의 Debian .deb 추출 방식 안 씀 (from-scratch 정체성).
#   ② **GSettings 정공 픽스** (x86 v7 "빈 독" 부검 결론): 40_maruxos.gschema.override로
#      dock-items 기본값 박제(chroot gsettings 실효값 검증 완료) + xinitrc
#      GSETTINGS_BACKEND=keyfile(영속·프로세스간 공유 — memconf=memory의 한계 해소).
#   ③ config v5: .desktop 3종 + dockitem 3종(skel+root) + tint2 systray-only(SC,
#      plank와 공존 — plank=bottom center, tint2=bottom right) + plank xinitrc 풀블록.
#   ④ 비컴포지트 환경 = plank 불투명 모드 (기능 정상, 줌/페이드 off — picom은 추후 결정).
#
# 사용법: 부팅 → startx → 하단 중앙 Plank 독(아이콘 3개: 터미널/파일/Firefox) 클릭 실행.
###############################################################################
set -euo pipefail
BUILD_TARGET="arm64"
...
trap cleanup EXIT
echo "===== MaruxOS 2.0.0 ARM64 v14 (배치 P: Plank dock) 빌드 $(date) ====="
