#!/bin/bash
# =============================================================================
# MaruxOS 2.0.0 ARM64 — 데스크톱 config populate v6 (배치 P2: Plank 폴리시)
# -----------------------------------------------------------------------------
# v5 대비 델타 (라이브 실험 2026-07-29 반영):
#   ① xinitrc에 **picom 기동** (plank 앞) — 반투명·라운드·줌은 컴포지터 필수
#      (라이브 실측: 비컴포지트에선 색톤만 반영, 라운드=사각 폴백).
#   ② tint2 panel_size 360→200 — 트레이 빈 상태서 시계가 왼쪽으로 치우치던
#      실기기 관찰 픽스 (v14 사진).
#   나머지는 v5 동일 (keyfile 백엔드, .desktop/dockitem 3종, HW커서, 한국어 시계).
# 실행 전제: .p-COMPLETE + .p2-COMPLETE (picom/Marux 테마 설치됨).
# =============================================================================
set -uo pipefail
...
sed -i \
  ...
  -e 's/^panel_size = .*/panel_size = 200 36/' \
