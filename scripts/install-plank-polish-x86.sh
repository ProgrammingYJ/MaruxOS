#!/bin/bash
# =============================================================================
# ⚠️ x86_64 변환본 (2026-08-28, x86 데스크톱 패리티) — 원본 = install-plank-polish-arm64.sh
#   /home/administrator/MaruxOS/x86-parity/rootfs-lfs-parity 에서 네이티브 chroot 빌드 (qemu 불필요, JOBS 32, lib64 pkgconfig, meson --libdir=lib)
# =============================================================================
# =============================================================================
...
if [ ! -f "$MARKDIR/picom" ]; then
  ...
  ( meson setup _build --prefix=/usr --libdir=lib --buildtype=release --wrap-mode=nofallback \
