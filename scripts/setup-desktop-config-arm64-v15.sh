#!/bin/bash
# =============================================================================
# MaruxOS 2.0.0 ARM64 — 데스크톱 config populate v15 (편집기 아이콘 + 배치 T 툴 4종)
# -----------------------------------------------------------------------------
# v14 대비 델타 (2026-08-27):
#   ① 편집기 아이콘 file-text.png → marux-editor.png ("줄 있는 종이", 사용자 선택, PIL 생성, DZN SSOT).
#   ② **배치 T**: LXImage-Qt(이미지 뷰어+스크린샷) · SpeedCrunch(계산기) · LXQt Archiver(압축) · qps(작업관리자)
#      .desktop 4종(config SSOT, PIL 아이콘 4종) + openbox 메뉴 + MIME(image/* → lximage-qt, 압축 → lxqt-archiver).
#      feh는 폴백 잔류. 독·바탕화면은 그대로(메뉴/파일 연결로 접근).
#   나머지는 v14 동일.
# v14 대비 델타 (2026-08-27): 편집기 아이콘을 플레이스홀더 file-text.png → **marux-editor.png**(디자이너 부재로
#   사용자가 고른 "줄 있는 종이 + 접힌 모서리" 스타일, PIL로 생성 128px, `MaruxOS 디자인/marux-editor.png`)로 교체.
#   배포: $DZN/marux-editor.png → /usr/share/pixmaps/maruxos/. featherpad.desktop(config SSOT)·idesk editor.lnk 갱신.
#   나머지는 v14 동일.
# (v14 델타 — 이력)
# v13 대비 델타 (2026-08-27, 출품 당일 — "txt 여는 notepad 같은 거"):
#   ① featherpad.desktop 배포(config SSOT) + openbox 메뉴 "Text Editor"(apps/root) + idesk 바탕화면 아이콘.
#   ② **MIME 기본앱**: /usr/share/applications/mimeapps.list + /etc/xdg/mimeapps.list 에 text/plain 등 →
#      featherpad. rootfs에 desktop-file-utils가 없어 mimeinfo.cache도 config가 직접 생성(GIO가 읽는 형식).
#      → PCManFM-Qt에서 .txt 더블클릭 = FeatherPad.
#   ③ 독은 3종 유지(터미널·파일·Firefox). 편집기는 메뉴/바탕화면/파일 연결로 접근.
#   나머지는 v13 동일.
# (v13 델타 — 이력)
...
[ -x "$LFS/usr/bin/featherpad" ] || { echo "🚨 ABORT: featherpad 없음(배치 E 미완 — install-featherpad-arm64.sh)"; exit 1; }
for t in lximage-qt speedcrunch lxqt-archiver qps; do [ -x "$LFS/usr/bin/$t" ] || { echo "🚨 ABORT: $t 없음(배치 T 미완 — install-lxtools-arm64.sh)"; exit 1; }; done
...
# v15: 편집기 아이콘 (DZN SSOT)
for ic in marux-editor marux-image marux-calc marux-archive marux-taskmgr; do
...
echo "  ✓ .desktop 10종 (xterm/mc/firefox/qterminal/pcmanfm-qt/featherpad/lximage-qt/speedcrunch/lxqt-archiver/qps)"
...
LNK
cat > "$LFS/etc/skel/.idesktop/editor.lnk" <<'LNK'
table Icon
  Caption: Text Editor
  Command: featherpad
  Icon: /usr/share/pixmaps/maruxos/marux-editor.png
  X: 80
  Y: 380
  Width: 48
  Height: 48
...
# ---------- 7b. MIME 기본앱: 텍스트 → FeatherPad (v14) ----------
# GIO(libfm-qt가 사용)는 mimeapps.list의 [Default Applications]를 먼저 보고, 후보 목록은 mimeinfo.cache에서 읽는다.
# rootfs에 update-desktop-database(desktop-file-utils)가 없으므로 mimeinfo.cache를 직접 생성한다(형식: [MIME Cache] type=app.desktop;).
echo "[7b] MIME 기본앱 (text/* → featherpad, image/* → lximage-qt, 압축 → lxqt-archiver)"
...
"
mkdir -p "$LFS/etc/xdg" "$LFS/usr/share/applications"
...
grep -q '^text/plain=.*featherpad.desktop' "$LFS/usr/share/applications/mimeinfo.cache" || { echo "🚨 ABORT: mimeinfo.cache에 text/plain→featherpad 없음"; exit 1; }
[ -f "$LFS/usr/share/mime/mime.cache" ] || echo "  (경고) shared-mime-info mime.cache 없음 — .txt 판별 불가"
echo "  ✓ mimeapps.list ×2 + mimeinfo.cache($(grep -c = "$LFS/usr/share/applications/mimeinfo.cache")항목)"
...
# ---------- 검증 ----------
echo "===== config populate v15 완료 ====="
echo "  자동로그인  : $(grep -c 'autologin root' "$LFS/etc/inittab") / startx프로필: $(grep -c '^  startx' "$LFS/root/.bash_profile")"
echo "  배치 T      : $(for t in lximage-qt speedcrunch lxqt-archiver qps; do [ -x "$LFS/usr/bin/$t" ] && printf "%s✓ " "$t" || printf "%s❌ " "$t"; done)/ image/png 기본앱: $(grep -c '^image/png=lximage-qt' "$LFS/etc/xdg/mimeapps.list") / zip: $(grep -c '^application/zip=lxqt-archiver' "$LFS/etc/xdg/mimeapps.list")"
echo "  편집기      : featherpad $([ -x "$LFS/usr/bin/featherpad" ] && echo ✓ || echo ❌) / text/plain 기본앱: $(grep -c '^text/plain=featherpad' "$LFS/etc/xdg/mimeapps.list") / 메뉴 Text Editor: $(grep -c featherpad "$LFS/etc/xdg/openbox/menu.xml")"
