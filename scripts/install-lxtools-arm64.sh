...
if [ -z "${SKIP_ARCHIVER:-}" ]; then
  ...
  if ! ls "$LFS/usr/lib/libjson-glib-1.0.so"* >/dev/null 2>&1; then
    if [ ! -f "$MARKDIR/json-glib" ]; then
      ...
      cat > "$TSRC/cross-aarch64.ini" <<CROSS
[binaries]
c = ['aarch64-linux-gnu-gcc', '--sysroot=$LFS']
cpp = ['aarch64-linux-gnu-g++', '--sysroot=$LFS']
...
CROSS
      cd "$TSRC/json-glib-$JV" && rm -rf _b \
        && meson setup _b --cross-file "$TSRC/cross-aarch64.ini" --prefix=/usr -Dintrospection=disabled -Dgtk_doc=disabled -Dman=false -Dtests=false >> "$LOG" 2>&1 \
  ...
  if [ ! -f "$MARKDIR/infozip" ]; then
    ...
    ZC="aarch64-linux-gnu-gcc --sysroot=$LFS"
    ok=1
...
for b in speedcrunch qps lximage-qt lxqt-archiver; do
  ...
  if [ -x "$LFS/usr/bin/$b" ]; then
    readelf -h "$LFS/usr/bin/$b" | grep -q AArch64 && echo "  ✅ $b (AArch64)" || { echo "  🚨 $b 아키텍처 오류"; ok=0; }
...
bash "$SCRIPTS/gate-qt-launch-arm64.sh" || { echo "🚨 ABORT: Qt 기동 게이트 실패"; ok=0; }
