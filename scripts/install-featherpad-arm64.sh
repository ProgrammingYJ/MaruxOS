...
grep -q -- '-D_FORTIFY_SOURCE=2' "$QTHOST/mkspecs/linux-aarch64-gnu-g++/qmake.conf" || { echo "🚨 ABORT: 호스트 mkspec FORTIFY 없음"; exit 1; }
...
if [ -x "$LFS/usr/bin/featherpad" ]; then
  readelf -h "$LFS/usr/bin/featherpad" | grep -q AArch64 && echo "  ✅ AArch64 ELF" || { echo "  🚨 아키텍처 오류"; ok=0; }
