#!/bin/bash
# MaruxOS 2.0.0 "Cooked" - cooked-v4 Build Script
# =================================================
# v3 부팅 결과: Plank가 SIGTRAP exit 133으로 silent 즉사 (GSettingsSchema 'net.launchpad.plank' not found).
# v4가 schema 배포/컴파일 strict + XDG 환경변수 + xterm Xresources 통합 fix.
#
...
# Plank가 있으면 tint2는 systray + clock만 담당하는 slim 버전
if [ -f "$SQUASHFS_ROOT/usr/bin/plank" ] && [ -f "$CONFIG_DIR/tint2/tint2rc-systray" ]; then
    ...
    echo "  ✓ tint2 → systray-only mode (Plank dock 활성)"
else
    cp "$CONFIG_DIR/tint2/tint2rc" "$SQUASHFS_ROOT/etc/xdg/tint2/tint2rc"
    echo "  ✓ tint2 → full panel mode (default)"
fi
