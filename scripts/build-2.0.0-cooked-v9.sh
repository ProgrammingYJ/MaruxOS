#!/bin/bash
# MaruxOS 2.0.0 "Cooked" - cooked-v9 Build Script (x86_64 공개 릴리즈 ISO)
# =================================================
# v9 (2026-08-27, GitHub 릴리즈용):
#   - 공유 config/openbox/menu.xml 은 ARM64 배치 Q~T로 Qt 앱(qterminal·pcmanfm-qt·featherpad…) 기준이 됨
#     → x86 rootfs엔 그 앱이 없어 메뉴가 깨진다. **x86 전용 config/openbox/menu-x86.xml**(origin/main 1.2.1판) 사용.
#   - root 비번을 공개 기본값 `marux`로 통일 (v8까지 빈 비번 + tty1 자동 로그인). 자동 로그인은 유지.
#   - 릴리즈 산출명 MaruxOS-2.0.0-x86_64.iso 로 복사 + sha256.
#   그 외 v8 동일.
# --- v8 헤더 ---
# MaruxOS 2.0.0 "Cooked" - cooked-v8 Build Script (Plank Rollback)
# =================================================
# v7 결과: Plank schema 해결 + dockitem 파일 위치 OK + 바이너리 실행 OK.
# 그러나 dock-items GSettings 키가 memconf 백엔드에서 주입 안 됨 → dock 비어있음.
# 디버그 가능하나 ARM64 (8/10 슬라이드 마감) 앞두고 시간 가치 안 맞음.
#
# v8 = Plank 롤백:
#   - Plank 미설치 (install-plank.sh 호출 안 함)
#   - tint2 → 풀 패널 모드 (1.x 시절 layout, 검증된 안정)
#   - config/plank/, install-plank.sh, tint2rc-systray 파일은 모두 유지 (deferred)
#     → 2.0.x 패치에서 GSettings 주입 디버그 후 부활 가능
#
# 발표 측면: Plank 챕터 = "schema 정공 + 14종 함정 일지" 이미 완성.
# dock 아이콘 표시 여부는 발표 임팩트와 무관.
#
# v7에서 가져오는 모든 변경사항 유지 (rollback 대상 아님):
#   - 커널 6.18.26 LTS
#   - 메타데이터 canonical 생성
#   - Xresources (xterm 한국어 fix)
#   - XDG 메타데이터 export
#   - .desktop StartupWMClass (Plank 없어도 tint2에서 효과)
# v5의 schema strict deploy가 "작동 안 한" 진짜 원인 발견 (2026-06-08 정찰):
#   - plank의 GSettings schema (net.launchpad.plank.gschema.xml)는 plank.deb / libplank1.deb에 없음.
#   - **별도 패키지 `libplank-common` (arch-independent, _all.deb)** 안에 박혀있음.
#   - install-plank.sh이 schema 스캔할 때 plank.deb와 libplank1.deb만 추출 → 어디에서도 schema 못 찾고 skip.
#   - 즉 schema 0개 deploy → 부팅 후 plank SIGTRAP.
#
# v6 변경사항 (vs cooked-v5):
#   - install-plank.sh의 DEBS 배열에 **`libplank-common_0.11.89-4_all.deb` 추가** (총 9 → 10개 .deb).
#   - schema 스캔 로직은 그대로 — libplank-common 추출본의 usr/share/glib-2.0/schemas/net.launchpad.plank.gschema.xml을 자동 매칭.
#   - tint2 풀 패널 모드는 v5 그대로 유지 (plank 떠도 풀 panel). Plank가 정상 동작 검증되면 v7부터 systray-only 분기 부활 고려.
#
# v5 변경사항 (vs cooked-v4):
#   - **tint2 항상 풀 패널 (default tint2rc) 사용** — plank 설치 여부에 관계없이.
#     plank dock이 안 뜨는 동안 우측 systray만 떠있는 "반쪽 UI" 해소.
#     install-plank.sh은 계속 돌아감 (plank는 ISO에 박힘) — xinitrc가 plank 시도하지만 안 떠도 tint2 풀이 받쳐줌.
#   - plank 디버그는 별도 트랙: build script와 무관.
#
# v4에서 가져오는 모든 변경사항 유지 (schema strict, XDG env, Xresources)
# v3 이하 변경사항 모두 유지.
#
# v4 변경사항 (vs cooked-v3):
#   - **install-plank.sh 강화 (schema critical 처리)**:
#     - schema XML force-overwrite (copy_safe의 "exists, skip" 우회)
#     - chroot glib-compile-schemas 출력 노출 + exit code 체크 (silent fail 차단)
#     - gschemas.compiled에 net.launchpad.plank 진입 검증 (없으면 abort)
#     - early-exit 로직 강화: binary 있어도 schema 결손 시 재배포
#   - **xinitrc에 XDG 메타데이터 export**:
#     - XDG_SESSION_TYPE/CLASS/DESKTOP, XDG_CURRENT_DESKTOP, DESKTOP_SESSION
#     - plank의 [WARN] 다수 제거 + 다른 데스크톱 도구 인식
#   - **xterm Xresources 통합 (Section 21-A 픽스)**:
#     - /etc/X11/Xresources 시스템 wide 배포 (faceName Monospace + utf8 + IME)
#     - xinitrc에서 xrdb -merge 호출
#     - launcher 통한 xterm 호출 시 한국어 표시 fix
#
# v3에서 가져오는 모든 변경사항 유지 (Plank dock, tint2-systray, .desktop StartupWMClass)
#
# v2에서 가져오는 모든 변경사항 유지:
#   - 부팅 splash 정정 (67 → Cooked)
#   - canonical /etc/*release 템플릿 생성
#   - sudo $USER 함정 SUDO_USER 폴백
#
# v1에서 가져오는 모든 변경사항 유지:
#   - 커널: 6.7.4 (genesis hallucination) → 6.18.26 LTS (정공)
#   - 코드네임: 67 → Cooked
#   - 버전: 1.2.1 → 2.0.0
#   - [KERNEL] 단계: 02-build-kernel.sh 산출물을 rootfs-lfs에 sync
#   - 6.7.4 잔재 → legacy-1.x-kernel/ 백업 (롤백 보존)
#   - 검증 게이트: 도구 / 디스크 / 산출물 / 모듈 핸드오프 / 커널 버전 매칭
#   - WSL native fs 강제 (Windows fs case-insensitive 함정 회피)
#   - 1.2.1의 모든 데스크톱 기능 보존 (idesk, openbox, tint2, ibus-hangul, marux-* 헬퍼)
#   - minimal busybox initrd 재사용 (lib/modules 없음 → 6.18.26과 호환)
#   - MaruxOS 디자인/ → rootfs 아이콘 동기화
...
# ============================================================================
# [7.5] Plank dock — v8 ROLLBACK: 미설치
# ============================================================================
# v3~v7에서 Plank schema 정복했으나 dock-items GSettings 주입 이슈로 미사용.
# install-plank.sh / config/plank/ / tint2rc-systray 모두 frozen artifact로 보존.
# 2.0.x 패치에서 부활 가능. 현재 v8은 1.x tint2 풀 패널만 사용.
echo "[7.5] Plank installation SKIPPED (v8 rollback decision)"
# Plank 잔재가 rootfs에 있으면 제거 (v3~v7 빌드 흔적 청소)
if [ -f "$SQUASHFS_ROOT/usr/bin/plank" ]; then
    echo "  - Removing legacy Plank binary + config from rootfs..."
    rm -f "$SQUASHFS_ROOT/usr/bin/plank"
    rm -rf "$SQUASHFS_ROOT/etc/skel/.config/plank"
    rm -rf "$SQUASHFS_ROOT/root/.config/plank"
    # Plank schema XML도 제거 (다음 빌드에서 부활 시 재배포 강제)
    rm -f "$SQUASHFS_ROOT/usr/share/glib-2.0/schemas/net.launchpad.plank.gschema.xml"
fi

# ============================================================================
# [8] Config files
# ============================================================================
echo "[8] Copying config files (xinitrc, openbox, scripts, Xresources)..."
cp "$CONFIG_DIR/xinitrc" "$SQUASHFS_ROOT/etc/X11/xinit/xinitrc"
chmod 755 "$SQUASHFS_ROOT/etc/X11/xinit/xinitrc"

# Xresources — XTerm 기본 폰트(Xft) 강제. launcher 통한 xterm 호출 시 한국어 표시 fix.
if [ -f "$CONFIG_DIR/Xresources" ]; then
    cp "$CONFIG_DIR/Xresources" "$SQUASHFS_ROOT/etc/X11/Xresources"
    chmod 644 "$SQUASHFS_ROOT/etc/X11/Xresources"
    echo "  ✓ /etc/X11/Xresources installed (XTerm Xft + UTF-8)"
fi

mkdir -p "$SQUASHFS_ROOT/etc/xdg/openbox"
cp "$CONFIG_DIR/openbox/rc.xml" "$SQUASHFS_ROOT/etc/xdg/openbox/rc.xml"
# v9: x86 전용 메뉴 (공유 menu.xml은 ARM64 Qt 앱 기준)
[ -f "$CONFIG_DIR/openbox/menu-x86.xml" ] || { echo "🚨 ABORT: config/openbox/menu-x86.xml 없음"; exit 1; }
cp "$CONFIG_DIR/openbox/menu-x86.xml" "$SQUASHFS_ROOT/etc/xdg/openbox/menu.xml"
grep -q 'qterminal\|pcmanfm-qt\|featherpad' "$SQUASHFS_ROOT/etc/xdg/openbox/menu.xml" && { echo "🚨 ABORT: x86 메뉴에 ARM64 전용 Qt 앱 참조"; exit 1; }
for cmd in xterm mc firefox; do [ -x "$SQUASHFS_ROOT/usr/bin/$cmd" ] || [ -L "$SQUASHFS_ROOT/usr/bin/$cmd" ] || { echo "🚨 ABORT: x86 메뉴 대상 $cmd 없음"; exit 1; }; done
# v9: root 비번 공개 기본값 marux (tty1 자동 로그인은 유지)
PWHASH=$(openssl passwd -6 "${MARUX_ROOT_PW:-marux}")
...
SALT=$(awk -F: '$1=="root"{print $2}' "$SQUASHFS_ROOT/etc/shadow" | cut -d'$' -f3)
[ "$(openssl passwd -6 -salt "$SALT" "${MARUX_ROOT_PW:-marux}")" = "$(awk -F: '$1=="root"{print $2}' "$SQUASHFS_ROOT/etc/shadow")" ] && echo "  ✓ root 비번 = marux" || { echo "🚨 ABORT: root 비번 검증 실패"; exit 1; }
...
# v8: tint2 항상 풀 패널 (Plank 롤백). 1.x 시절 검증된 안정 layout.
cp "$CONFIG_DIR/tint2/tint2rc" "$SQUASHFS_ROOT/etc/xdg/tint2/tint2rc"
echo "  ✓ tint2 → full panel mode (v8 rollback)"
chmod 644 "$SQUASHFS_ROOT/etc/xdg/tint2/tint2rc"

# Plank dock 설정 미배포 (v8 rollback). 잔재가 rootfs에 있으면 [7.5]에서 청소됨.
