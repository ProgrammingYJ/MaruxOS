#!/bin/bash
# =============================================================================
# MaruxOS 2.0.0 ARM64 — 데스크톱 config populate v11 (터미널 xterm → QTerminal)
# -----------------------------------------------------------------------------
# v10 대비 델타 (2026-08-25, 배치 Q 완주 반영 — 2.0.0 로드맵 3번):
#   ① **기본 터미널을 qterminal로 교체** — .desktop/dockitem/openbox 메뉴/idesk 아이콘.
#      xterm은 **제거하지 않고 폴백으로 유지**(Qt 스택 문제 시 대피로 + 회귀 비교용).
#   ② **Qt 런타임 환경변수** — 플러그인 경로를 명시(`QT_QPA_PLATFORM_PLUGIN_PATH`).
#      Qt는 컴파일 시 박힌 경로(/usr/plugins)를 쓰지만, 명시하면 탐색 실패를 원천 차단한다.
#      (xcb 플러그인을 못 찾으면 "could not load the Qt platform plugin"으로 앱이 죽는다)
#   나머지는 v10 동일 (tint2 은퇴/통합 상태 바, 볼륨 부트스트랩, picom, keyfile, HW커서).
...
for d in xterm mc firefox qterminal; do
  cp -f "$CFG/applications/$d.desktop" "$LFS/usr/share/applications/$d.desktop"
done
echo "  ✓ xterm/mc/firefox/qterminal.desktop"
...
for d in qterminal mc firefox; do
...
rm -f "$LFS/etc/skel/.config/plank/dock1/launchers/xterm.dockitem"       "$LFS/root/.config/plank/dock1/launchers/xterm.dockitem" 2>/dev/null || true
echo "  ✓ 3 dockitem × 2경로 (터미널=qterminal)"
...
echo "  .desktop    : $(ls "$LFS/usr/share/applications/"{xterm,mc,firefox,qterminal}.desktop 2>/dev/null | wc -l)/4"
...
echo "  Qt 터미널   : $([ -x "$LFS/usr/bin/qterminal" ] && echo "qterminal ✓" || echo "없음 ❌") / 플러그인경로 $(grep -c QT_QPA_PLATFORM_PLUGIN_PATH "$LFS/etc/X11/xinit/xinitrc")"
