#!/bin/bash
###############################################################################
# MaruxOS 2.0.0 "Cooked" — ARM64 이미지 빌드 v15 (배치 P2: Plank 폴리시 — picom + Marux)
#
# v14 대비 변경 (라이브 실험 2026-07-29 확정값 박제):
#   ① **picom v11.2 컴포지터** (xrender+vsync 미니멀 — 그림자/페이드 off) + 의존 6종
#      (libev/libconfig/uthash/xcb-util-{renderutil,image,wm}). plank 반투명·라운드·
#      줌 애니메이션 발현 + 티어링 방지 보너스.
#   ② **Marux 테마** — macOS풍: TopRoundness=10 + 반투명 화이트(248/235 a150) +
#      HorizPadding=5(긴 바) + ItemPadding=4. override theme='Marux'.
#      (라이브 실측: 색톤은 즉시 반영됐고 라운드는 컴포지터 필수 = 사각 폴백 확인)
#   ③ config v6: xinitrc picom 기동(plank 앞) + tint2 panel_size 360→200(시계
#      왼쪽 치우침 픽스 — v14 실기기 사진).
#
# 사용법: 부팅 → startx → 라운드·반투명 Marux 독 + 아이콘 줌 애니메이션.
###############################################################################
set -euo pipefail
BUILD_TARGET="arm64"
...
trap cleanup EXIT
echo "===== MaruxOS 2.0.0 ARM64 v15 (배치 P2: picom+Marux 테마) 빌드 $(date) ====="
...
grep -q "^panel_items = SC" "$LFS/etc/xdg/tint2/tint2rc" || { echo "🚨 ABORT: tint2 systray-only 미적용"; exit 1; }
# 🔴 배치 P2: 폴리시 (v15 신규)
...
grep -q "picom" "$LFS/etc/X11/xinit/xinitrc" || { echo "🚨 ABORT: xinitrc picom 기동 없음(config v6 미적용)"; exit 1; }
grep -q "^panel_size = 200 36" "$LFS/etc/xdg/tint2/tint2rc" || { echo "🚨 ABORT: tint2 panel_size 200 미적용"; exit 1; }
