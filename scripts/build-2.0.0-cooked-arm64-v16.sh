#!/bin/bash
###############################################################################
# MaruxOS 2.0.0 "Cooked" — ARM64 이미지 빌드 v16 (v15 실기기 라이브픽스 4종 박제)
#
# v15 대비 변경 (2026-07-29 실기기 라이브픽스 — 사용자 "창전환 유리 룩 다 됨" 판정):
#   ① **클릭 창전환 픽스** — rc.xml에 Client 컨텍스트 마우스바인드(레포 원본 수정 =
#      x86 트랙 잠복버그 동시 해결. 첫 창겹침 테스트서 발각)
#   ② **bamfdaemon 세그폴트 픽스** — 근원: 빌드순서 실수로 wnck가 SN 미포함 컴파일
#      → bamf가 wnck_handle_set_default_icon_size서 즉사(dmesg 트레이스 특정).
#      wnck(SN 포함)+bamf 재빌드 + xinitrc 명시 기동(D-Bus runner=우분투 전용이라 우회)
#   ③ **유리 확정값** — 라운드 10 + 알파 95(실기기 판정. "회색"=어두운 배경 광학이었음)
#   ④ **HDMI 정리** — cmdline서 미연결 HDMI-A-2 모드강제 제거(vc4 RGB 경고 스팸 원인
#      유력) + xinitrc dmesg -n 1(콘솔 스팸 차단, syslog는 유지)
#
# 사용법: 부팅 → startx → 유리 독 + 클릭 창전환 + 독 실행 점 표시.
###############################################################################
set -euo pipefail
BUILD_TARGET="arm64"
...
trap cleanup EXIT
echo "===== MaruxOS 2.0.0 ARM64 v16 (라이브픽스 4종 박제) 빌드 $(date) ====="
