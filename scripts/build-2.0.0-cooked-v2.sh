#!/bin/bash
# MaruxOS 2.0.0 "Cooked" - cooked-v2 Build Script
# =================================================
# v1 부팅 검증 통과 (커널 6.18.26 + cgroup + 네트워크 + 한국어 + dmesg 무회귀) 후,
# v1에서 발견된 메타데이터 잔재 정리.
#
# v2 변경사항 (vs cooked-v1):
#   - 부팅 splash "67" 잔재 제거 → 코드명 "Cooked" 일관 표시
#   - /etc/{os,maruxos,issue,lsb}-release 파일을 sed가 아닌 canonical 템플릿으로 통째 생성
#     (v1: VERSION_ID=1.0 처럼 unquoted 변형이 sed에서 누락되는 함정 노출)
#   - VERSION_CODENAME / DISTRIB_CODENAME 필드 명시적 채움
#   - sudo 실행 시 $USER=root 변환 함정 → SUDO_USER 폴백 (v1 도중 적용분 유지)
#
