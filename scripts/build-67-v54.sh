#!/bin/bash
# MaruxOS 67-v54 Build Script (GTK3 immodules cache update for Korean input)
...
echo "GTK3 immodules cache + hangul input fix"
echo "=========================================="

cd "$WORK_DIR"

# Clean up any existing mounts
echo "[1/12] Cleaning up..."
...
echo "[2/12] Setting up ISO directory..."
...
echo "[3/12] Installing ibus-hangul Korean input method..."
...
# Apply v54 changes (GTK3 immodules cache)
echo "[4/12] Copying config files..."

# Copy xinitrc (with GTK immodules cache update + hangul initial mode)
echo "  - Copying xinitrc with GTK immodules cache update..."
...
echo "[5/12] Adding full Korean locale support..."
...
echo "[6/12] Configuring ibus-hangul..."
...
# [NEW in v54] GTK3 immodules cache update (CRITICAL!)
echo "[7/12] Updating GTK3 immodules cache (CRITICAL for Korean input)..."

if [ -x "$SQUASHFS_ROOT/usr/bin/gtk-query-immodules-3.0" ]; then
    echo "  - Running gtk-query-immodules-3.0 (stdout redirect)..."
    chroot "$SQUASHFS_ROOT" /bin/bash -c '/usr/bin/gtk-query-immodules-3.0 > /usr/lib/gtk-3.0/3.0.0/immodules.cache 2>/dev/null' || true
    echo "    ✓ GTK3 immodules cache updated"
...
echo "[8/12] Copying desktop files..."
...
echo "[9/12] Copying tint2 config..."
...
# Remove old squashfs
echo "[11/12] Removing old squashfs..."
...
echo ""
echo "Changes in v54:"
echo "  ✅ GTK3 immodules cache 업데이트 (핵심 수정!)"
echo "     - gtk-query-immodules-3.0 --update-cache 실행"
echo "     - GTK 앱(Firefox 등)이 im-ibus.so를 인식하도록 함"
echo "  ✅ xinitrc에 런타임 immodules cache 업데이트 추가"
echo "  ✅ hangul initial-input-mode를 'hangul'로 변경"
echo "     - Ctrl+Y 누르면 바로 한글 입력 모드로 전환"
echo ""
echo "Root cause (v53에서 한/영 전환 안됨):"
echo "  - im-ibus.so 파일은 존재했지만"
echo "  - GTK3 immodules.cache가 생성되지 않아서"
echo "  - GTK 앱들이 im-ibus.so를 로드하지 못함"
echo "  - 키 이벤트가 ibus에 전달되지 않았음"
echo ""
echo "사용법:"
echo "  - Ctrl+Y: 한/영 전환"
echo "  - 한글 타이핑: 2벌식 QWERTY"
echo "  - F9: 한자 변환"
echo "=========================================="
