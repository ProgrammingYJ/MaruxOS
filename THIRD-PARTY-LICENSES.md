...
| Artwork by **tuna27** — **only** these four: logo (`marux-logo*.png`, `marux_logo.svg`), wallpaper (`marux-desktop.png`), terminal icon (`marux-terminal.png`), file-manager icon (`marux-file-manager.png`). Every other file in `MaruxOS 디자인/` is MaruxOS's own work (Unlicense, row above). | © tuna27 — licensed to MaruxOS for redistribution; **license to be confirmed by the designer** (target: Unlicense or CC BY 4.0). Until confirmed these are *not* part of the public-domain dedication. |
| Linux From Scratch (book instructions) | MIT (instructions) / CC BY-NC-SA 2.0 (book text). MaruxOS scripts are our own transcription of the build steps. |
