...
6. **디자이너 자산**: tuna27 배경화면·아이콘은 디자이너 저작권 → Unlicense/CC-BY 동의 확인 필요(미확인 상태에선 헌정에서 제외 명시). → **2026-08-28 해결**: tuna27이 오픈소스 배포 동의 → 4종(로고·배경화면·터미널·파일관리자 아이콘)을 **CC BY 4.0**으로 확정(저작권 포기 아님 → Unlicense 미편입, 저작자 표시 유지). `config/licenses/CC-BY-4.0.txt` 추가(다음 이미지 빌드부터 동봉).
...
| 7 | featherpad/lxtools가 `.q-FORTIFY2-OK`(ARM64 재빌드 마커)를 전제 → x86엔 재빌드가 없어 ABORT | **`gate-qt-fortify-x86.sh`** 신설: 재빌드 대신 *검증*(gcc-13 래퍼 암묵 FORTIFY=3 실측 → qtbase corelib Makefile·CMake 캐시 4종에 `=2` 실재 → 기동 게이트) 후 SHA 마커 기록. 첫 판은 빌드 디렉토리를 `build/`로 가정해 캐시 0개인데도 통과 → "0개=실패"로 고침(게이트 자기 오류) |
| 8 | NEEDED 검사 `ls "$LFS/usr/lib/$n" "$LFS/lib/$n"` — x86은 `/lib`이 실제 디렉토리라 하나만 없어도 실패 → 전멸 오판(featherpad `FEATHERPAD_COMPLETE=NO`) | v33 교훈 #2 재발. 경로별 `[ -e ]` 개별 검사(+lib64) |
| 9 | **x86 rootfs의 glib 이중 상태**: `/usr/lib` = **2.80**(1.x Plank 사고 때 Ubuntu deb에서 들어온 바이너리 — 헤더·pc 없음, ld.so.cache가 먼저 잡는 실제 런타임) / `/usr/lib64` = **2.78.4**(LFS 정공 빌드 — 헤더 `/usr/include/glib-2.0`·`glib-2.0.pc`·glibconfig.h는 이쪽). 기존 프로그램 전부 "2.78 헤더로 컴파일 → 2.80 런타임"으로 살아있었음 | lxqt-archiver만 깨짐: FindGLIB이 `/usr/lib64/libglib-2.0.so`(2.78) 절대경로 + `-lgobject-2.0`은 -L 순서상 `/usr/lib`(2.80) → 2.80 gobject가 2.80 glib 심볼 요구 → undefined. 픽스 3단: ① lib64의 glib 계열 `.so`+`.pc` **격리**(`$B/lib64-quarantine/`, 런타임 해석은 이미 /usr/lib이라 회귀 0 — plank·quicksettings·bamf·pcmanfm-qt·firefox NEEDED 검증) ② `glibconfig.h`(2.78)를 `/usr/lib/glib-2.0/include/`에 복사(FindGLIB 경로) + lib64/glib-2.0/include 복원(libfm-qt export가 절대경로로 요구) ③ **libfm-qt CMake export**(`/usr/share/cmake/fm-qt/fm-qt-targets.cmake`)가 `/usr/lib64/lib{glib,gio,gobject,gthread}-2.0.so`를 절대경로로 박음 → lib64에 dev 심볼릭링크 → `../lib`(2.80). 결과 archiver 링크 = 전부 2.80 일관. 툴체인에도 `FIND_LIBRARY_USE_LIB64_PATHS OFF`+`CMAKE_IGNORE_PATH lib64`(단독으론 부족했음 — pkg_check_modules의 HINTS는 무시 안 됨) |
| 10 | 다른 glib 사용 앱(libfm-qt·pcmanfm-qt·plank)은 왜 멀쩡했나 | pkg-config `-l` 경로만 써서 -L 순서상 /usr/lib 2.80 계열로 **일관** 링크됐기 때문. 절대경로+`-l` 혼용이 유일한 지뢰 |
| 11 | 배치 X(x86 전용 신설 `install-x86-extras.sh`): 퀵설정 볼륨 백엔드 amixer/aplay 부재 + ibus-hangul 한/영 노출 패치 미적용 | x86 rootfs는 gcc 13.2 네이티브라 **chroot 네이티브 빌드**: alsa-lib 1.2.10(헤더 없어서 같이) + alsa-utils 1.2.10 + ibus-hangul 1.5.5(공개 `patches/ibus-hangul-1.5.5-export-input-mode.patch`를 `patch -p1`로 적용 — 공개 diff와 바이트 일치 보장, 실패 시 등가 sed 폴백+경고). 마커 `$B/.x-COMPLETE` |

### ✅ 완료 (2026-08-28 10:10 KST) — cooked-v10 = x86 데스크톱 패리티 ISO
- **`MaruxOS-2.0.0-cooked-v10.iso` 238M**(248,672,256 B), SHA **`2b7f70b6e315d5e24d3926cde44e64602e6374b290d2ee4f95888cfec645c762`** = 릴리즈 사본 `MaruxOS-2.0.0-x86_64.iso`. v9 1.24G → **-81%**(스테이징 슬림 871M → squashfs xz 212M).
- **QEMU/KVM(WSL, `apt qemu-system-x86`) 검증 — 모니터 `screendump -f png`로 스크린샷 판독**: 전원 → tty1 자동 로그인 → X → 데스크톱(배경·idesk 4종·Plank 독 3종·우상단 상태 바) / `sendkey`로 Win+T → **QTerminal**, Win+E → **PCManFM-Qt**(rc.xml 키바인드를 xterm/mc → Qt로 통일, 재빌드 반영) / QTerminal에서 **Shift+Space 한글 "안녕"** 입력 실증(Qt5 ibus 플러그인) / `featherpad &` → FeatherPad(한국어 UI) / 독 실행중 인디케이터. 마우스 주입(`mouse_move/button`)은 게스트에 안 닿아 키보드 경로만 사용.
- 빌드 중 게이트가 실제로 잡은 것(전부 스크립트 교정): ① 상속 dangling 모듈 18개(원본 v9 rootfs부터 깨진 gio/imlib2/cups/pulse/cairo-script 참조) → **leaf 모듈 자동 삭제(다중 패스)**, 실행파일이면 ABORT ② 호스트에서 돌린 `find -xtype l`이 `/usr/lib/libpciaccess.so.0 → /usr/lib64/…` *절대* 심볼릭링크를 dangling 오판·삭제 → **Xorg ldd 게이트가 잡음** → chroot 내부 실행 ③ `soname=$(…|grep)` set -e 침묵사(v33 교훈 재발) ④ sed 치환식 경로 `/` 충돌 ⑤ 스테이지 `umount dev: busy` → lazy 폴백 + "아직 마운트됨" ABORT 게이트(호스트 /dev 유입 방지).
- 도구 사고 2건(교훈): Windows `python3`는 스토어 스텁이라 **조용히 실패**(첫 FORTIFY 게이트 패치가 안 들어간 채 "0개 통과") / perl `s|…|`에 `\|`가 섞이면 패턴이 깨져 **파일 머리에 치환문이 삽입**(build v10·config·fortify 3회) → 스크립트 편집은 Edit 도구/줄번호 스플라이스만.
- **(다음 빌드 메모, 2026-08-29)** 설명문 `LICENSE`→`LICENSING.md`, `LICENSE`=Unlicense 원문(GitHub 감지용). v34/v10 스크립트는 `$WINROOT/LICENSE`만 이미지에 넣으므로 **다음 버전부터 `LICENSING.md`도 `/usr/share/licenses/MaruxOS/`에 복사**할 것(게이트 `MaruxOS/LICENSE` 존재 검사는 그대로 통과).
- **(다음 빌드 메모, 2026-08-29)** 설명문 `LICENSE`→`LICENSING.md`로 이름 변경, `LICENSE`=Unlicense 원문(GitHub 라이선스 감지가 `other`였던 원인 = 설명문을 라이선스 파일로 읽음). v34/v10 빌드 스크립트는 `$WINROOT/LICENSE`만 이미지에 넣으므로 **다음 버전부터 `LICENSING.md`도 `/usr/share/licenses/MaruxOS/`에 복사**할 것(기존 `MaruxOS/LICENSE` 존재 게이트는 그대로 통과).
- **릴리즈 v2.0.0 x86 자산 교체 완료(10:15)**: `MaruxOS-2.0.0-x86_64.iso` 248,672,256 B + sha256 사이드카(`2b7f70b6…`) + 릴리즈 노트 갱신(패리티·238 MB·ALSA 주의). 남은 것: 사용자 실기기(PC) 부팅은 선택 / 오디오: Live ISO는 ALSA 직결이라 QEMU intel-hda에서 "♪ --"(카드 드라이버 =m 여부 미확인 — 블로커 아님).

### 이전 상태 (2026-08-28 09:20 KST 기준 — 최종 사슬 빌드 중)
- ✅ Qt5 / plank 사슬 / mc·feh / quicksettings / pcmanfm-qt / picom(p2) / **featherpad(.e)** / **lxtools 4종(.t)** — 기동 게이트 7종 PASS (`lxtools.console`)
- ✅ 스크립트 신설: `gate-qt-fortify-x86.sh`, `install-x86-extras.sh`, **`setup-desktop-config-x86-v1.sh`**(ARM64 v15 이식 — inittab 기존 autologin 인정 / `.bash_profile` exec→일반 / xorg.conf.d는 flat 가속만(Pi 전용 swcursor 제외) / startx `sysxinitrc` 실측 배포 / tint2 은퇴), **`build-2.0.0-cooked-v10.sh`**(PRE 게이트 = 마커·아키텍처·FORTIFY SHA·config 바이트 일치·wnck 실해석·ime 마커·라이선스·sanitize → 커널 sync → locale/캐시/메타/비번 → **스테이징 슬림 rsync + strip + 라이선스 + NEEDED/python 게이트 + 슬림 사본 네이티브 chroot 기동 게이트(bind-mount로 mountpoint 조건 충족)** → mksquashfs xz → ISO → 릴리즈 사본)
- 🔄 **09:17 발사**: `install-x86-extras` → `setup-desktop-config-x86-v1` → `gate-qt-fortify-x86` → `build-2.0.0-cooked-v10` (콘솔 `$B/extras.console`·`config-x86-v1.console`·`fortify2.console`·`build-v10.console`, 마지막 줄 `FINAL_CHAIN_RC=`)
- ⏳ 이후: QEMU 부팅 검증(`-device intel-hda -device hda-duplex`로 볼륨 백엔드까지) → 릴리즈 x86 자산 교체(`gh release upload --clobber`) + 릴리즈 노트/README "x86은 초기 데스크톱" 문구 삭제 → ISO-HISTORY cooked-v10 SHA 확정
- 확인 명령: scratchpad `x86-final-status.sh` — 없으면 `tail -3 $B/{extras,config-x86-v1,fortify2,build-v10}.console; ls $B/.{x,cfg-x86-v1}*`

### 이전 상태 (2026-08-28 00:55 KST 기준)
